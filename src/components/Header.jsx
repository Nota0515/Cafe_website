    ...
    <div className='relative w-full h-20'>
      <div className='fixed top-0 left-0 w-full h-20 z-50 px-5 py-4 bg-n-2 border-b border-b-n-3'>
        <div className='relative flex flex-row justify-center gap-2 md:gap-5 w-full h-10'>
          ...
          <nav className={`${openNavigation ? 'flex' : "hidden"} fixed backdrop-blur-sm bg-n-3/10 top-0 left-0 right-0 bottom-0 lg:static md:static md:mx-auto lg:flex md:flex lg:mx-auto`}>
            <div className={`navelements relative z-2 flex flex-col gap-20 md:gap-5 lg:gap-10 items-center justify-center m-auto md:flex-row lg:flex-row`}>
              {navigation.map((item)=>(
                <span key={item.id}>
                  <a className='text-n-1 text-2xl font-poppins' href={item.url} onClick={handleclick}>{item.title}</a>
                </span>
              ))}
            </div>
          </nav>
